-- 022_add_activo_platos.sql
-- Añade columna activo a platos para controlar visibilidad en dashboard
-- Actualiza vista_coste_platos para incluir activo

ALTER TABLE platos ADD COLUMN IF NOT EXISTS activo BOOLEAN DEFAULT true;

UPDATE platos SET activo = true WHERE activo IS NULL;
...
GROUP BY p.id, p.nombre, p.precio_venta, p.margen_objetivo, p.categoria, p.restaurante_id, p.factor_porcion, p.receta_id, rb.porciones_base, p.activo;
